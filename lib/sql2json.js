/**  Скажем спасибо https://github.com/martindrapeau/csvjson-app */
		...
		for (l = 0; l < lines.length; l++) {
			...
			if (words.length >= 3 &&
				...
				_.forEach(values, function(value) {
					...
					var second = _.trim(words[1]);
					if (_.startsWith(first, "'") || _.startsWith(first, "`") || _.startsWith(first, '"')) {
						tables[name].header.push(_.trim(first, "`'\""));
						if (_.startsWith(second, 'int')
							|| _.startsWith(second, 'float') 
							|| _.startsWith(second, 'datetime') 
						) {
							tables[name].fieldTypes.push('number');
						} else {
							tables[name].fieldTypes.push('string');
						}
					}
				}, []);
	...
	_.each(tables, function(table, name) {
		...
		objects[name] = _.map(table.values, function(values) {
			...
			for (var k=0; k < keys.length; k++) {
				...
				if (o[keys[k]] !== null && table.fieldTypes[k] === 'number') {
					o[keys[k]] = +o[keys[k]];
				}
			}
			return o;
